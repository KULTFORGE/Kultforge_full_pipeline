...
// Automatically retries on 529 (overloaded) up to 3 times with backoff.
async function callClaude(input, maxTokens) {
  ...
  const delays = [3000, 8000, 15000]; // ms between retries
  let lastError;
  ...
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    ...
    try {
      ...
      if (res.status === 529) {
        lastError = new Error("API overloaded — retrying (" + attempt + "/" + delays.length + ")...");
        continue;
      }
      ...
      if (e.message && e.message.includes("retrying")) { lastError = e; continue; }
      throw e;
    }
  }
  throw lastError || new Error("API overloaded after retries — try again in a moment");
}
